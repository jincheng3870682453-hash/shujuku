...
> 单文件交互式演示页 [`demo.html`](./demo.html)，无需安装、无需后端，浏览器直接打开即可体验项目全部亮点。

| 章节 | 内容 |
|:---|:---|
| 🏠 Hero 首屏 | 星空粒子跟随鼠标 + **标题探照灯**（鼠标划过标题，光圈内文字实时反色发光，rAF 插值平滑跟手） |
| 📊 核心数据 | 170 并发压测 · 42ms 平均响应 · 25 版本迭代 · 9 种质感等核心指标一屏速览 |
| ⭐ 十大亮点 | 动态字段 / 双数据库 / 三级审核 / 多模型 AI / 语音输入 / 桌面端等 |
| ⚡ 性能优化 | 分页查询 · React Query 缓存 · 防抖机制 · 渲染优化四板斧 |
| 🖼 项目截图 | 登录页 / 数据登记 / 统计分析 / 审核中心 / 用户管理 实拍 |
| 🌈 质感系统 | 9 种背景质感 + 四色自定义配色实时切换（全 CSS 变量驱动） |
| 🗂 系统架构 | 7 张 Mermaid 架构图：架构 / 时序 / ER / 权限 / 状态机 / 组件树 / 部署 |
| 🔐 权限模型 | 4 级角色 · 21 项权限 层级可视化 |
| ⏳ 迭代历程 | 12 天 25 版本时间轴 + 开发手记入口 |
| 🚀 部署形态 | Web / 桌面 / Docker / 打包 4 种部署方案 |

**快速打开方式**

- 直接双击 `demo.html`（推荐，零依赖）
- 或启动本地静态服务：
```bash
python -m http.server 8090 --directory .
# 浏览器访问 http://localhost:8090/demo.html
```

> 提示：Hero 首屏的标题探照灯在支持 `mix-blend-mode` 与 `clip-path` 的现代浏览器（Chrome / Edge / Firefox / Safari）中体验最佳。
...
| 功能 | 说明 |
|:---|:---|
| 主题配置 | 设置页新增「外观与主题」：主题色、质感（Glass 玻璃拟态 / Neon 霓虹 / Flat 扁平）、玻璃透明度、霓虹强调色、背景图 |
| 浮动主题按钮 | 页面内一键切换主题 |
| 按用户隔离 | 主题配置存入 `users.theme` 字段，不同账号互不影响，登录后自动恢复 |
...
| 修复 | 说明 |
|:---|:---|
| HR 删除绕过审核 | HR 的单条删除与批量删除此前会直接生效，现改为必须提交审核，由管理员（boss）审核通过后才会真正执行删除 |
| 员工规则不变 | 员工（employee）仍遵循全局审核开关的规则 |
...
| 话术定制 | AI 按角色注入不同分析视角：员工聚焦个人改进、HR 人员管理与组织视角、管理员系统治理与全局决策 |
| 身份提示 | AI 分析页面新增「当前身份 + 数据范围」提示条 |
...
一次"闲着没事干"的念头——看到字段写死的登记系统想加列却加不了，索性自己动手写一个动态字段的数据登记系统。从第一天起就借助 AI 协作开发，逐步成长为具备完整权限体系、审核工作流与 AI 分析能力的一体化平台。

### 关键里程碑

| 时间 | 里程碑 | 产出 |
|:---|:---|:---|
| 第 1 天 | MVP 跑通 | 动态字段 + 数据录入 + 列表展示 |
| 第 2-3 天 | 经历重构阵痛 | 学会版本管理，确立"一个功能一验证"的开发节奏 |
| 第 4-5 天 | 听取社区反馈 | 采纳"SQLite 太单调、UI 太难看"建议，升级工具链 |
| 第 6 天 | 双数据库支持 | SQLite + MySQL 双引擎跑通 |
| 第 7 天 | 压力测试 | 170 并发 · 72 小时 0 失败 · 平均 42ms |
| 第 8 天 | UI 全面优化 | 8 种质感、自定义背景、完善协议文档 |
...
1. **尽早引入版本管理**——经历过"去回收站翻备份"才被迫学会 git，越早越好
2. **增量开发**——一个功能加一个验证，避免一次性堆积导致全线崩溃
3. **虚心对待反馈**——两条社区建议最终都落地为核心功能
4. **量力而行**——在不熟悉的工具链上先摸清工作流，再全面铺开
